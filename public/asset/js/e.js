...
break;}
return arr;}
function _365webcall_getNewSubmitForm(action){var __oHtml=document.documentElement;var __oBody;if(typeof(ducument)!='undefined'&&typeof(ducument.body)!='undefined')
...
function _365webcall_createNewFormElement(inputForm,elementName,elementValue){var newElement=document.createElement("input");newElement.setAttribute("type","hidden");newElement.setAttribute("id",elementName);newElement.setAttribute("name",elementName);newElement.setAttribute("value",elementValue);inputForm.appendChild(newElement);return newElement;}
function _365webcall_getSource(div_Description){var inputArray=document.getElementById(div_Description).getElementsByTagName("input");var textArray=document.getElementById(div_Description).getElementsByTagName("textarea");var content=document.getElementById(div_Description).innerHTML;var inp=new RegExp("(<input(((?!type).)*type=[\"']?text[\"']?[^>]*[/]*((?!value).)*=[\"']?[\"']?[^>]*[/]*?)>)","ig");content=content.replace(inp,function($,$2){return"##input##"});for(var i=0;i<inputArray.length;++i){if(inputArray[i].type=="text"){content=content.replace("##input##","<input type=\"text\" value=\""+inputArray[i].value+"\"/>");}}
var inp=new RegExp("<textarea[^>]*>([^<]+)</textarea>","ig");content=content.replace(inp,function($,$2){return"##text##"});for(var i=0;i<textArray.length;++i)
content=content.replace("##text##","<textarea>"+textArray[i].value+"</textarea>");return content;}